...
const Threads = ({ color = [1, 1, 1], amplitude = 1, distance = 0, enableMouseInteraction = false, ...rest }) => {
  ...
  useEffect(() => {
    ...
    let active = true; // prevents async contextlost from a cleaned-up instance cancelling a new RAF
    ...
    // Position canvas to fill parent absolutely
    ...
    function handleMouseLeave() { targetMouse.current = [0.5, 0.5]; }
    ...
    function update(t) {
      ...
      program.uniforms.iTime.value = (t - startTime.current) * 0.001;

      if (enableMouseInteraction) {
        const s = 0.05;
        currentMouse.current[0] += s * (targetMouse.current[0] - currentMouse.current[0]);
        currentMouse.current[1] += s * (targetMouse.current[1] - currentMouse.current[1]);
        program.uniforms.uMouse.value[0] = currentMouse.current[0];
        program.uniforms.uMouse.value[1] = currentMouse.current[1];
      }
