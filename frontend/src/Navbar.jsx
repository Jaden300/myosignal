...
import { IconGear, IconBook, IconBulb, IconPeople, IconRocket, IconBuilding, IconMail, IconDemo, IconCode, IconChart } from "./Icons"
  ...
  return (
    <div ref={ref} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} style={{ position:"relative" }}>
      <button style={{
        ...
        {label}
        <span style={{ fontSize:9, opacity:0.5, display:"inline-block", transform:open?"rotate(180deg)":"rotate(0deg)", transition:"transform 0.2s" }}>▾</span>
      </button>
      ...
      <div style={{
        ...
        minWidth:210, padding:"6px", zIndex:200,
        pointerEvents: open?"auto":"none",
        opacity: open?1:0,
        transform: open ? "translateX(-50%) translateY(0) scale(1)" : "translateX(-50%) translateY(-6px) scale(0.97)",
        ...
        {items.map(([label, path, Icon]) => (
          <button key={path} onClick={()=>{ navigate(path); setOpen(false) }} style={{
            ...
            cursor:"pointer", fontFamily:"var(--font)", fontSize:14, fontWeight:400,
...
function NavLink({ label, path, pathname, accent }) {
  ...
  return (
    <span onClick={()=>navigate(path)} style={{
      fontSize:14, fontWeight: accent?600:400, cursor:"pointer",
  ...
  return (
    <nav style={{
      ...
      <div style={{ display:"flex", alignItems:"center", gap:24 }}>
        ...
        <div style={{ display:"flex", background:"var(--bg-secondary)", border:"1px solid var(--border)", borderRadius:100, padding:2 }}>
          {["en","fr"].map(l=>(
            <button key={l} onClick={()=>switchLang(l)} style={{
              background: lang===l?"#fff":"transparent", border:"none", borderRadius:100,
              padding:"3px 10px", fontSize:12, fontWeight:lang===l?600:400,
              color: lang===l?"var(--text)":"var(--text-tertiary)",
              cursor:"pointer", fontFamily:"var(--font)",
              boxShadow: lang===l?"0 1px 4px rgba(0,0,0,0.08)":"none", transition:"all 0.15s"
